...
function createGist(token, dir) {
  ...
  opts.body = {
    public: false,
    description: 'just another work in my gist studio',
    files: ['index.js', 'package.json', 'open.js', 'save.js'].reduce((m, v) => {
        m[v] = {
          content: fs.readFileSync(path.join(__dirname, '..', 'base', v)).toString()
        };
        return m;
      }, {})
  };
...
function newUser() {
  console.log('It looks like you haven\'t set Gist Studio up for gist support yet');
